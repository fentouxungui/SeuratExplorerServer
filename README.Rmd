...
Report files are published into the app's `www/reports/` directory. Files on the same filesystem are linked with hard links (no extra disk usage), while files on a different filesystem are copied; copies larger than `MaxReportFileSize` (default 100 MB) are skipped to avoid filling up disk. The links are served both by Shiny Server's static `www` handling and via `shiny::addResourcePath()`, so report downloads work under local `runApp()` and Shiny Server (including versions ≥ 1.5.7 that reject symlinks pointing outside `site_dir`).

### 💬 Comments / 留言

A built-in message board lets users leave comments on individual datasets:

- Commenter identity is taken from the Shiny login name (read-only), with an optional free-text real name.
- Comments can target a specific dataset and reply to a previous comment; the reply thread is shown as an indented tree.
- Users can post and reply; the technician (login name in `TechnicianUser`) can additionally mark comments as resolved or delete them.
- Comments are stored in a CSV file (see `CommentsFile`), with file-locked, atomic writes so concurrent users are safe.
...
- **Dimension Reduction**: Default visualization method (UMAP, t-SNE, PCA, etc.)
...
**[Demo Database](http://netinfo.nibs.ac.cn:666/SeuratExplorerServer-Index/)**
...
```{r echo=FALSE, out.width="100%", fig.cap="Browse and download analysis reports"}
knitr::include_graphics("inst/extdata/www/comments.png")
```
...
**Symptoms**: Report browser shows "No reports available" or report links return "Not Found"
...
- **Large files are skipped**: files that need to be copied (cross-filesystem) and exceed `MaxReportFileSize` (default 100 MB) are silently skipped. Raise the limit with `launchSeuratExplorerServer(MaxReportFileSize = 200 * 1024^2)` or disable it with `Inf`.
- **Write permission**: the Shiny process must be able to write `<app_dir>/www/reports/`. Check `sudo chown -R shiny:shiny <app_dir>/www` if report files are not created.
- **Shiny Server ≥ 1.5.7**: report files are published as hard links / copies inside `www/reports/` (not symlinks), so they are served correctly on current Shiny Server versions.
...
- **在线演示**: [http://netinfo.nibs.ac.cn:666/SeuratExplorerServer-Index/](http://netinfo.nibs.ac.cn:666/SeuratExplorerServer-Index/)
